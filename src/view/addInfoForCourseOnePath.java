...
public class addInfoForCourseOnePath extends JPanel {
	...
	private int coutse_id;
	private Course course;
	private CourseDAO course_dao;
	int active_path_for_stady = 0;
	 ...
	 * Create the panel.
	 */
	public addInfoForCourseOnePath(Main pFrame, int pCoutse_id) {
		coutse_id = pCoutse_id;
		frame = pFrame;
		
		try {
			settingsRead = new XMLSettings();
		} catch (Exception e) { 
			JOptionPane.showConfirmDialog(null, "Не найден файл с настройками", "Ошибка", JOptionPane.PLAIN_MESSAGE);
		}
	...
	private void initComponents() {
		...
		JLabel label_8 = new JLabel("Курсовые: ");
			...
			gl_panel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel.createSequentialGroup()
					...
					.addGroup(gl_panel.createParallelGroup(Alignment.LEADING)
						.addGroup(gl_panel.createSequentialGroup()
							.addGap(307)
							.addComponent(label, GroupLayout.PREFERRED_SIZE, 188, GroupLayout.PREFERRED_SIZE)
							.addContainerGap())
						.addGroup(gl_panel.createSequentialGroup()
				...
				.addGroup(gl_panel.createSequentialGroup()
					.addContainerGap()
					.addComponent(label_8, GroupLayout.PREFERRED_SIZE, 87, GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(ComponentPlacement.UNRELATED)
					.addGroup(gl_panel.createParallelGroup(Alignment.LEADING, false)
						.addComponent(radioCoursovy_project, 0, 0, Short.MAX_VALUE)
						.addComponent(radioCoursovy_job, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
						.addComponent(radioCoursovy_none, GroupLayout.PREFERRED_SIZE, 84, GroupLayout.PREFERRED_SIZE))
					.addGap(110)
					.addComponent(btnSave, GroupLayout.PREFERRED_SIZE, 124, GroupLayout.PREFERRED_SIZE)
					.addGap(181))
				.addGroup(gl_panel.createSequentialGroup()
					.addGap(45)
					.addComponent(error, GroupLayout.PREFERRED_SIZE, 476, GroupLayout.PREFERRED_SIZE)
	...
	private void afterInit() {
		...
		course = course_dao.get(coutse_id);
		if(course.getStady_in_path_1() == 1) {
			active_path_for_stady = 1;
		} else if(course.getStady_in_path_2() == 1){
			active_path_for_stady = 2;
		} else if(course.getStady_in_path_3() == 1){
			active_path_for_stady = 3;
		} else if(course.getStady_in_path_4() == 1){
			active_path_for_stady = 4;
		}
		lblStady_path_1.setText("Path " + active_path_for_stady);
		
		group_name = course.getGroups();
		full_hours = course.getHours();
		int count_credits = 0;
		int auditor_hours = 0;
		...
		input_auditor_hours.setText(Integer.toString(auditor_hours));//fdfsdfdfd
		...
		btnSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(valid_form) {
					...
					courseInfo.setCourse(coutse_id);
